/* ================================================================
   PI THINKING — Aluno Engine v3.3
   Correções: chave participant-id correta, reset de state entre logins,
   getLocalCompletions não contamina mais o state
   ================================================================ */
(function (global) {
  'use strict';
  ...
  var state = {
    ...
    ferramentas: [],
    completions: {},
    controls: { fases_liberadas: [1], ferramentas_liberadas: [] }
  };
  ...
  function notify() {
    ...
    subscribers.forEach(function (cb) { cb(snapshot); });
    global.dispatchEvent(new CustomEvent('pi-aluno-engine-updated', { detail: snapshot }));
  }

  function loadCompletions() {
    /* ── FIX 1: usa apenas PIToolBridge.getCompletedIndex() que já usa
       a chave com participant-id. Não mistura com getLocalCompletions()
       que lê a chave global antiga. ── */
  ...
  function getCurrentPhase(ferramentas) {
    ...
    var nextOpen = list.find(function (t) { return t.desbloqueada && !t.concluida; });
    if (nextOpen) return nextOpen.fase;
    var lastIdx = getLastCompletedIndex();
    if (lastIdx >= 0) return list[lastIdx] ? list[lastIdx].fase : 1;
    return 1;
  }
  ...
  function getJornadaDisponivel(ferramentas) {
    var list = ferramentas || computeFerramentas();
    return list.length > 0 && list.every(function (t) { return t.concluida; });
  }

  function getState() {
    if (!state.percurso) return null;
    var ferramentas = computeFerramentas();
    var concluidas = ferramentas.filter(function (t) { return t.concluida; }).length;
    ...
    return {
      ...
      jornada: global.PIConfig && typeof global.PIConfig.getPostWorkshopTool === 'function'
        ? global.PIConfig.getPostWorkshopTool() : null,
      jornadaDisponivel: getJornadaDisponivel(ferramentas),
      controls: clone(state.controls),
      totalFerramentas: total,
      totalConcluidas: concluidas,
      progresso: progresso,
      faseAtual: getCurrentPhase(ferramentas),
      concluido: total > 0 && concluidas === total,
      proximaFerramenta: getProximaFerramenta(ferramentas),
      perfil: {
        /* ── FIX 2: usa a chave correta pi-session-participant-id ── */
  ...
  function completeTool(toolId, payload) {
    ...
    if (global.PIToolBridge && typeof global.PIToolBridge.markComplete === 'function' &&
        !global.PIToolBridge.isCompleted(id)) {
      global.PIToolBridge.markComplete(id, payload || {}, { skipSync: false });
    }
    if (global.PISync && typeof global.PISync.updateParticipantProgress === 'function' &&
  ...
  /* ── FIX 3: reset limpa completamente o state em memória ── */
  function reset() {
    state.completions = {};
    state.controls = { fases_liberadas: [1], ferramentas_liberadas: [] };
    state.percurso = null;
    state.percursoKey = '';
    state.ferramentas = [];
    if (stopControlsWatcher) { stopControlsWatcher(); stopControlsWatcher = null; }
    notify();
    return true;
  }
  ...
  function startWatchingControls() {
    if (stopControlsWatcher) { stopControlsWatcher(); stopControlsWatcher = null; }
    if (global.PISync && typeof global.PISync.watchControls === 'function' &&
        global.PISync.isInSession && global.PISync.isInSession()) {
      stopControlsWatcher = global.PISync.watchControls(function (controls) {
        state.controls = Object.assign(
          { fases_liberadas: [1], ferramentas_liberadas: [] },
          controls || {}
        );
        notify();
      });
  ...
  /* ── FIX 4: init sempre reseta o state antes de carregar novos dados ── */
  function init(percursoKey) {
    if (!global.PIConfig) throw new Error('PIConfig não carregado.');
    /* Limpa estado em memória para evitar contaminação entre logins */
    state.completions = {};
    state.controls = { fases_liberadas: [1], ferramentas_liberadas: [] };
  ...
  function setPercurso(percursoKey) { return init(percursoKey); }

  function getFerramenta(toolId) {
    var id = normalizeId(toolId);
    var s = getState();
    return s ? s.ferramentas.find(function (t) { return t.id === id; }) || null : null;
  }

  function getFerramentasPorFase(phaseNumber) {
    var s = getState();
    return s ? s.ferramentas.filter(function (t) { return Number(t.fase) === Number(phaseNumber); }) : [];
  }

  global.AlunoEngine = {
    init: init, restore: restore, reset: reset, subscribe: subscribe,
    getState: getState, setPercurso: setPercurso, completeTool: completeTool,
    isCompleted: isCompleted, isUnlocked: isUnlocked,
    getFerramenta: getFerramenta, getFerramentasPorFase: getFerramentasPorFase
  };
